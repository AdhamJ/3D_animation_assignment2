	...
	Eigen::RowVectorXd& p) 
{
	...
	//p = (V.row(E.row(e)[0]) + V.row(E.row(e)[1])) / 2;
	//p = (V.row(E.row(e)[0]) + V.row(E.row(e)[1])) / 2;
	//std::cout << v << std::endl;;
	//std::cout << "\n============\n" << "Q1 :\n" << Q1 << "\n Q2:\n" << Q2 << "\n v:\n" << v << "\n cost:\n" << cost << "\n============\n";
}
...
namespace glfw
{
  ...
  IGL_INLINE bool Viewer::load_mesh_from_configuration() {
	...
	if (!infile) {
		...
		while (getline(infile, mesh_path)) // To get you all the lines.
		{
			std::cout << "opening " << mesh_path << std::endl;
			this->load_mesh_from_file(mesh_path);
			this->init_simplefication_objs();
		}
		infile.close();
		return true;
	}
